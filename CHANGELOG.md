...
+ [新增] 视频创作台按模型能力表门控：分辨率只渲染站点 /api/pricing 枚举值（枚举缺失或模型档位固定时隐藏选择器），时长按模型窗口或离散档（5|10 等）呈现，固定分辨率模型不再显示自由输入。
+ [新增] 参考图经站点素材通道直传（presign → 直传 → 确认）换成交互公开读 URL 后提交，门控模型改用 JSON 负载引用素材 URL，附上传进度、失败重试与「素材约 3 小时后自动删除」提示。
+ [新增] 交叉约束：grok-imagine-video-1.5 挂参考图后自动隐藏 1080p，素材数量超限在提交前拦截。
